...
FMApp.controller('FileManagerCtr', ['$scope', '$http', '$location',
  function ($scope, $http, $location) {
    ...
    var baseurl = 'http://10.2.253.121:8090/api/'+userid;
    //var baseurl = 'http://localhost:8080/api/'+userid;
    var hash2paths = function (relPath) {
      var paths = [];
      var names = relPath.split('/');
      var path = '#/';
      paths.push({name: '主目录', path: path});
    ...
    FM.clickFile = function (file) {
      if (file.folder) {
        $location.path(decodeURIComponent(file.relPath));// open folder by setting url hash
      }
      else {
        downloadFile(file);
      }
